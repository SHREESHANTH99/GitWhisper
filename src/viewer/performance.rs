pub fn show_performance(path: &str) {
    let report = match crate::analysis::performance_analyzer::analyze_target(path) {
        Ok(report) => report,
        Err(error) => {
            eprintln!("{error}");
            return;
        }
    };

    println!("Performance Report for {}\n", report.target);
    println!(
        "Overall risk: {}/100 | Files analyzed: {}\n",
        report.overall_risk, report.files_analyzed
    );

    if !report.findings.is_empty() {
        println!("Findings:");
        for finding in &report.findings {
            println!("- {}", finding);
        }
        println!();
    }

    for file_report in &report.file_reports {
        println!(
            "{}\n  Risk: {}/100 | Complexity: {} | Nested loops: {} | Allocations: {} | I/O signals: {} | Churn: {}",
            file_report.path,
            file_report.risk_score,
            file_report.approx_complexity,
            file_report.nested_loop_signals,
            file_report.allocation_signals,
            file_report.io_signals,
            file_report.recent_churn
        );
        for finding in file_report.findings.iter().take(3) {
            println!("  Finding: {}", finding);
        }
        if let Some(suggestion) = file_report.suggestions.first() {
            println!("  Suggestion: {}", suggestion);
        }
        println!();
    }

    if !report.suggestions.is_empty() {
        println!("Recommended next steps:");
        for suggestion in &report.suggestions {
            println!("- {}", suggestion);
        }
    }
}
