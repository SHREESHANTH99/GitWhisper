...
pub fn prepare_commit_report(
    ...
    let commit_message =
        crate::git::commit_message(&resolved_commit).unwrap_or_else(|_| crate::git::CommitMessage {
            subject: String::new(),
            body: String::new(),
        });
...
fn build_annotation_prompt(
    ...
    prompt.push_str("Focus on what changed, why it likely changed, and the most relevant impact.\n");
    prompt.push_str("Only make claims supported by the evidence. If uncertain, say likely.\n");
    prompt.push_str("Return only a concise 1-2 sentence summary with no bullets and no heading.\n\n");
...
mod tests {
    ...
    fn compacts_whitespace_for_note_text() {
        assert_eq!(compact_text("hello   world\nfrom\tgitwhisper", 50), "hello world from gitwhisper");
    }
}
